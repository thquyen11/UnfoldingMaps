 ...
 * team
 * 
 * @author Ho Thanh Quyen Date: July 17, 2015
 */
public class EarthquakeCityMap extends PApplet {
	...
	public void draw() {
		...
		// If user click on the city, pop up earthquake info
		if (lastClicked != null && lastClicked.getClass() == CityMarker.class) {
			addPointInfo();
		}
	...
	private void sortAndPrint(int numToPrint) {
		...
		for (int i = 0; i < quakeArray.length - 1; i++) {
			for (int j = i + 1; j < quakeArray.length; j++) {
				if (quakeArray[j].compareTo(quakeArray[i]) == 1) {
					EarthquakeMarker temp = quakeArray[j];
					quakeArray[j] = quakeArray[i];
					quakeArray[i] = temp;
				}
			}
		}

		// Print out numtoPrint biggest number
	...
	// helper method to draw info when user click on a point in GUI
	private void addPointInfo() {
		...
		textAlign(LEFT, CENTER);
		textSize(12);
		text("Earth Quake Warning", xbase + 25, ybase + 25);
		...
		fill(0, 0, 0);
		textAlign(LEFT, CENTER);
		// Popup the number of nearby earthquake (within threatCircle)
		text("Quantity: "+numberOfEarthQuake().size(), tri_xbase + 15, tri_ybase);
		// Popup the average magnitude of nearby earthquake
		text("Avg. mag: "+averageQuakeMagnitude(numberOfEarthQuake()), xbase + 50, ybase + 70);
		fill(255,0,0);
		ellipse(xbase + 35, ybase + 70, 10, 10);
	}

	private float averageQuakeMagnitude(List<Marker> list) {
		// TODO Auto-generated method stub
		float sum = 0;
		for (Marker marker : list) {
			System.out.println(marker.getProperty("magnitude").toString());
			sum += Float.parseFloat(marker.getProperty("magnitude").toString());
		}

		return list.size()!=0?sum / list.size():0;
	}

	private List<Marker> numberOfEarthQuake() {
		// TODO Auto-generated method stub
		List<Marker> quakeEffectCity = new ArrayList<>();
		for (Marker mhide : quakeMarkers) {
			EarthquakeMarker quakeMarker = (EarthquakeMarker) mhide;
			if (quakeMarker.getDistanceTo(lastClicked.getLocation()) <= quakeMarker.threatCircle()) {
				quakeEffectCity.add(mhide);
			}
		}
		return quakeEffectCity;
	}
